...
/**
 * An enumeration of the different "pages" displayed in the tabs presented in
 * the main activity. These values include extra details for each page, such
 * as the icon to be used. The order of the values in this {@code enum} is
 * the order in which the pages should be displayed (left-to-right, each in
 * its own tab).
 *
 * @author damo
 */
public enum TimerPage {
     ...
     */
    TIMES_LIST(TimerListFragment.class,
        R.drawable.ic_format_list_bulleted_white_24dp),

    /**
     * The page showing a chart of solve times and a table of solve-time
     * statistics.
     */
    CHART_AND_STATS(TimerGraphFragment.class,
        R.drawable.ic_timeline_white_24dp);

    /**
     * All of the values in this {@code enum}. This is used internally for
     * more efficient conversion of a tab position (zero-based) to a
     * {@code TimerPage} value.
     */
     ...
     *
     * @param fragmentClass
     *     The fragment class used to represent this timer page. The class must
     *     implement a public, static, no-argument {@code newInstance} method.
     * @param iconResID
     *     The resource ID of a drawable to use as the icon representing this
     *     page.
     */
    ...
    /**
     * Creates a new instance of the fragment used to present the user interface
     * for this timer page.
     *
    ...
    public BaseMainFragment createPageFragment() {
        try {
            ...
            return (BaseMainFragment) newInstanceMethod.invoke(
                null, (Object[]) null);
        } catch (Exception e) {
            throw new RuntimeException(
                "Fragment for TimerPage is not defined properly.", e);
        }
    }

    /**
     * Gets the timer page that corresponds to the given tab position. An error
     * will occur if the tab position is outside of the valid range.
     *
