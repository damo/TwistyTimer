...
/**
 * The colors of the faces of a puzzle. Only 6-faced puzzles are supported. This enum also supports
 * translation of the default face colors for Skewb and Square-1 puzzles to match the default
 * colors for all cube puzzles and the management of custom colors.
 *
 * @author damo
 */
public enum FaceColor {
     ...
     * default colors of an NxNxN cube, where the latter are ordered by the values of this enum.
     */
    ...
    private static final FaceColor[] SKEWB_FACE_COLOR_MAP = {
            UP,    // white  (Skewb UP    has same default color as NxNxN cube UP)
            DOWN,  // yellow (Skewb DOWN  has same default color as NxNxN cube DOWN)
            BACK,  // orange (Skewb BACK  has same default color as NxNxN cube LEFT)
            FRONT, // red    (Skewb FRONT has same default color as NxNxN cube RIGHT)
            LEFT,  // green  (Skewb LEFT  has same default color as NxNxN cube FRONT)
            RIGHT, // blue   (Skewb RIGHT has same default color as NxNxN cube BACK)
    };

    /**
     * The faces of a "Square-1" puzzle in the order in which their TNoodle default colors match
    ...
    private static final FaceColor[] SQUARE1_FACE_COLOR_MAP = {
            ...
            FRONT, // red    (Square-1 FRONT has same default color as NxNxN cube RIGHT)
            RIGHT, // green  (Square-1 RIGHT has same default color as NxNxN cube FRONT)
            LEFT,  // blue   (Square-1 RIGHT has same default color as NxNxN cube BACK)
    };
    ...
    /**
     * The default color of this face as a hexadecimal color string. The value has six hexadecimal
     * digits (letters are upper-case), two each for red, green and blue. There is no leading '#'.
     */
    ...
    /**
     * Converts the given color value to a hexadecimal color string. The string will have six
     * hexadecimal digits, two each for reg, green and blue. There will be <i>no</i> leading '#'
     * character.
     *
    ...
    /**
     * Gets the face color that is presented by the view with the given ID when customising the
     * colors through the user interface.
     *
    ...
    public static FaceColor forViewID(int viewID) {
        ...
        // This is not likely to be reached, so a guaranteed non-null return value makes it easier
        // to use this method elsewhere.
        return UP;
    }

    /**
     * Gets the resource ID of the view used to present this color in the user interface.
     *
    ...
    /**
     * Gets the color to use for this face expressed as a hexadecimal string. The value will be the
     * current custom value from the shared preferences, or the default value if no custom value is
     * defined. The string will have six hexadecimal digits, two each for reg, green and blue.
    ...
    /**
     * Gets the color value to use for this face. The value will be the current custom value from
     * the shared preferences, or the default value if no custom value is defined.
     *
    ...
    /**
     * Sets a new custom color value to use for this face. The value will saved to the shared
     * preferences, replacing any value already there.
     *
    ...
    /**
     * Resets the custom color value for this face, restoring the default color value. If the
    ...
    /**
     * Resets the custom color values for all faces, restoring the default color value to each
    ...
    /**
     * <p>
     * Gets the face for a given puzzle type that has the same default color as an NxNxN cube
     * puzzle has for this face.
     * </p>
     * <p>
     * This is required due to inconsistencies in the TNoodle library. For example, for all TNoodle
     * NxNxN cube puzzles, the default front face color is green. Therefore, if users customises
     * the shade of green, they would expect that the change would apply to the green face of other
     * types of puzzles. However, TNoodle defines the default face colors in different ways for the
     * other supported six-faced puzzles: "Skewb" and "Square-1". For example, TNoodle assigns
     * green to the left face of "Skewb" and to the right face of "Square-1".
     * </p>
     * <p>
     * For consistency, the NxNxN cube puzzles are used as a reference. When applying a custom
     ...
     *
     * @return
     *     The face of the given type of puzzle that has the same default color as this face of
