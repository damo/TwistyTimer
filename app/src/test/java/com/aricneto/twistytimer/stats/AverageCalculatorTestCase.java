...
public class AverageCalculatorTestCase {
    ...
    /**
     * Tests that times are rounded per WCA regulations as they are added, that these rounded times
     * are used to calculate the averages, and that the averages are also rounded before they are
     * returned.
     *
    ...
    public void testRounding() throws Exception {
        ...
        // Original values sum to 8,129, but they should be truncated *before* being summed.
        assertEquals(8_110, ac.getTotalTime());
        // Mean of original values if 8,129 / 3 = 2,709.666. Means are rounded to nearest multiple
        // of 10 ms, which would be 2,710. However, that would be *wrong*, as the mean should be
        // calculated from the sum of the *truncated* result times, so it is 8,110 / 3 = 2,703.333,
        // which is rounded to 2,700.
        ...
        // Now try that with larger values around 10 minutes. Those under 10 minutes still truncate
        // to a multiple of 10 ms, but those over 10 minutes *round* to a multiple of 1,000 ms.
...
fail("Write me!");
        ...
        // Original values sum to 1,799,975, which would truncate to 1,799,970, but they should be
        // truncated/rounded *before* being summed. Therefore, the sum is 1,800,410, which is
        // rounded down to 1,800,000
        assertEquals(1_800_000110, ac.getTotalTime());
        // Mean of original values if 8,139 / 3 = 2,713. Means are rounded to nearest multiple of
        // 10 ms, which would be 2,710. However, that would be *wrong*, as the mean should be
        // calculated from the sum of the *truncated* result times, so it is 8,110 / 3 = 2,703.333,
        // which is rounded to 2,700.
        assertEquals(2_700, ac.getMeanTime());
        assertEquals(2_700, ac.getCurrentAverage());
    ...
    public void testAverageOfThreeDisqualifyDNFs() throws Exception {
        ...
        // IMPORTANT: Rounding is to the nearest 1/100th of a second (10 ms) for averages less than
        // ten minutes in duration. Averages are also calculated after rounding the solve times.
        ...
        assertEquals(TIME_DNF, ac.getCurrentAverage()); // Last three were 100, 900, DNF.
        assertEquals(300, ac.getBestAverage());
        ...
        assertEquals(370, ac.getMeanTime()); // 3300 / 9 non-DNF solves. 366.6666... is truncated.
    }
    ...
    public void testAverageOfThreeAllowDNFs() throws Exception {
        ...
        assertEquals(480, ac.getCurrentAverage()); // Last three were 150, DNF, 800. Ignore DNF.
        ...
        assertEquals(TIME_DNF, ac.getCurrentAverage()); // Average of DNF, DNF, DNF. Ignore DNFs.
    ...
    public void testAverageOfFiveDisqualifyDNFs() throws Exception {
        ...
        assertEquals(280, ac.getCurrentAverage()); // (250+400+200) / 3. Exclude 150, 500.
        ...
        assertEquals(470, ac.getCurrentAverage()); // (400+200+800) / 3. Exclude 150, DNF.
        assertEquals(280, ac.getBestAverage());
    ...
    public void testAverageOfFiveAllowDNFs() throws Exception {
        ...
        assertEquals(TIME_DNF, ac.getCurrentAverage()); // 300 is only non-DNF time left.
        assertEquals(280, ac.getBestAverage());
    ...
    public void testAverageOfNDetailsForThreeAllowDNFs() throws Exception {
        ...
        assertEquals(ac.getN(), aoN.getTimes().length);
        assertArrayEquals(new long[] { 250, 150, TIME_DNF }, aoN.getTimes()); // mNext == 1
        ...
        assertEquals(ac.getN(), aoN.getTimes().length);
        assertArrayEquals(new long[] { 150, TIME_DNF, TIME_DNF }, aoN.getTimes()); // mNext == 2
        ...
        assertEquals(ac.getN(), aoN.getTimes().length);
        assertArrayEquals(new long[] { TIME_DNF, TIME_DNF, TIME_DNF }, aoN.getTimes()); // mNext==3
    ...
    public void testAverageOfNDetailsForFiveAllowDNFs() throws Exception {
        ...
        assertEquals(370, aoN.getAverage()); // Mean of 500+250+350. 150 and 600 are eliminated.
        ...
        assertEquals(ac.getN(), aoN.getTimes().length);
        assertArrayEquals(new long[] { 150, 250, 600, 350, TIME_DNF }, aoN.getTimes()); // mNext==1
        ...
        assertEquals(ac.getN(), aoN.getTimes().length);
        assertArrayEquals(new long[] { 250, 600, 350, TIME_DNF, TIME_DNF }, aoN.getTimes());
        // mNext == 2
        assertEquals(480, aoN.getAverage()); // Mean of 600+350. 250, DNF1 eliminated. DNF2 ignored.
        ...
        assertEquals(ac.getN(), aoN.getTimes().length);
        assertArrayEquals(new long[] { 600, 350, TIME_DNF, TIME_DNF, TIME_DNF }, aoN.getTimes());
        // mNext == 3
        ...
        assertEquals(ac.getN(), aoN.getTimes().length);
        assertArrayEquals(new long[] { 350, TIME_DNF, TIME_DNF, TIME_DNF, TIME_DNF },
        ...
        assertEquals(ac.getN(), aoN.getTimes().length);
        assertArrayEquals(new long[] { TIME_DNF, TIME_DNF, TIME_DNF, TIME_DNF, TIME_DNF },
